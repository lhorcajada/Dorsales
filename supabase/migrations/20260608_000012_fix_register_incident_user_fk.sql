-- Fix register_incident so incidents can be stored even when profile row is not available yet.
-- Root cause: incidents.user_id references profiles(id), so inserting p_user_id without an existing
-- profile fails on FK validation.
